...
class PomoTimer:
    ...
    def __init__(self):
        ...
        self.short_break_timer_label = ttk.Label(self.tab2, text="05:00", font=("Calibri", 48))
        self.short_break_timer_label.pack(pady=20)

        self.long_break_timer_label = ttk.Label(self.tab3, text="15:00", font=("Calibri", 48))
        ...
        self.pomo_counter_label = ttk.Label(self.grid_layout, text="Pomodoros: 0", font=("Calibri", 16))
        self.pomo_counter_label.grid(row=1, column=0, columnspan=3, pady=10)
    ...
    def start_timer(self):
        ...
        if timer_id == 1:
            ...
            while full_seconds > 0 and not self.stopped:
                ...
                self.pomo_timer_label.config(text=f"{minutes:02d}:{seconds:02d}")
                self.root.update()
                time.sleep(1)
                full_seconds -= 1
            if not self.stopped or self.skipped:
                self.pomo += 1
                self.pomo_counter_label.config(text=f"Pomodoros: {self.pomo}")
                if self.pomo % 4 == 0:
                    self.tabs.select(2)  
                else:
                    self.tabs.select(1)
    ...
    def reset_clock(self):
        ...
        self.pomo_counter_label.config(text="Pomodoros: 0")
        self.running = False
